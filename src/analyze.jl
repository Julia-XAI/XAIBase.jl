...
Apply the analyzer `method` for the given input, returning an [`Attribution`](@ref).
If `output_selection` is specified, the attribution will be calculated for that output.
Otherwise, the output with the highest activation is automatically chosen.

See also [`Attribution`](@ref).
"""
function analyze(
...
function (method::AbstractXAIMethod)(input; kwargs...)
    return analyze(input, method, MaxActivationSelector(); kwargs...)
end
function (method::AbstractXAIMethod)(input, output_selector; kwargs...)
    return analyze(input, method, output_selector; kwargs...)
end
